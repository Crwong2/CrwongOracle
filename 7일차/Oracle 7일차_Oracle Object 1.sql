...
--   불편하므로 사용하는 객체임
SELECT * FROM DBA_SYS_PRIVS -- System계정 실행(red) GRANT 뒤에 PRIVILEGE명을 붙여줘야 함.
ORDER BY 2 ASC;
...
-- - KEY와 VALUE형태롤 생성이 되며 KEY에는 인덱스로 만들 컬럼값, VALUE에는 행이 저장된
--   주소값이 저장됨
