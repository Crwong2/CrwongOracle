...
--a. 숫자 처리 함수
-- - ABS(절댓값), MOD(나머지), TRUNC(소숫점 지정 버림), FLOOR(버림), ROUND(반올림), CEIL(올림)
SELECT TRUNC(SYSDATE-HIRE_DATE,1) FROM EMPLOYEE;
...
SELECT MONTHS_BETWEEN(SYSDATE, '24/05/07') -- 두 날짜 사이에 몇개월인지
FROM DUAL; 
...
SELECT EMP_NAME "사원이름", HIRE_DATE "입사일", LAST_DAY(HIRE_DATE) AS "입사월 마지막일" FROM EMPLOYEE;
-- EXTRACT 년도, 월, 일을 DATE에서 추출해줌
SELECT EXTRACT(YEAR FROM SYSDATE) FROM DUAL;
SELECT EXTRACT(MONTH FROM SYSDATE) FROM DUAL;
SELECT EXTRACT(DAY FROM SYSDATE) FROM DUAL;
-- ex4) EMPLOYEE 테이블에서 사원이름, 입사년도, 입사월, 입사일을 조회하시오
SELECT EMP_NAME "이름"
