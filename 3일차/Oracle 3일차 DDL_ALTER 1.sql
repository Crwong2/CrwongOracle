...
-- 제약조건 추가, 삭제, 수정
-- 제약조건을 추가하면 이름이 생김, 이름을 알아야 삭제가 가능.
-- 2. 실행하면 ROW가 없음
...
-- 1. 실행하면 ROW가 1개 있음
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'USER_TBL';
-- 2. CONSTRAINT_NAME 이 FK_GRADE_CODE인 것 확인.

-- ex) USER_GRADE가 가지고 있는 제약조건 조회하여 PK삭제 후 같은 이름으로 재생성
-- 제약조건 수정은 삭제하고 추가하는 것, 단 이름은 바꿀 수 있음
ALTER TABLE USER_GRADE
RENAME CONSTRAINT PK_GRADE_CODE TO PRIMARYKEY_GR_CODE;
ALTER TABLE USER_GRADE
RENAME CONSTRAINT PRIMARYKEY_GR_CODE TO PK_GRADE_CODE;
-- 조회
...
-- ex03) USER_FOREIGN_KEY 테이블에 REG_DATE DATE 컬럼의 자료형을 VARCHAR2(10)으로 바꿔주세요(컬럼수정)
ALTER TABLE USER_FOREIGN_KEY
...
-- USER_DATE VARCHAR2(10) 컬럼의 자료형을 DATE 로 바꿔주세요
ALTER TABLE USER_FOREIGN_KEY
...
-- 제약 조건 활성화/비활성화
ALTER TABLE USER_TBL
DISABLE CONSTRAINT FK_GRADE_CODE;  -- 비활성화
ALTER TABLE USER_TBL
ENABLE CONSTRAINT FK_GRADE_CODE; -- 활성화
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'USER_TBL'; -- STATUS 창에서 확인.
COMMIT;
