...
SELECT EMP_NAME "사원명",
    ...
    END "부서명" -- CASE문을 활용해서 부서코드가 D5이면 총무부, D6이면 기획부, D9이면 영업부 작성
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', 'D6', 'D9')  -- 부서코드가 D5, D6, D9인 직원의 정보만 조회
ORDER BY DEPT_CODE ASC; -- 부서코드 기준 오름차순 정렬
...
--단, join과 IN 사용할 것
SELECT EMP_NAME "사원명", JOB_NAME "직급명", SALARY "급여", NVL(BONUS, 0) "보너스"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE BONUS IS NULL
