...
FROM EMPLOYEE
WHERE SALARY > 3047663;  -- 직접 평균급여를 구한 후 조건절에 쓴다.
...
-- @실습문제2
-- Asia1지역에 근무하는 직원의 정보(부서코드, 사원명)을 출력하세요
...
JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
JOIN LOCATION ON LOCATION_ID = LOCAL_CODE
WHERE LOCAL_NAME IN (SELECT LOCAL_NAME FROM LOCATION WHERE LOCAL_NAME IN ('Asia1'));
...
SELECT * FROM EMPLOYEE WHERE EXISTS (SELECT 1 FROM EMPLOYEE); -- RESULT SET이 나오므로 EXISTS가 FRUE를 반환 -> 메인쿼리의 결과값 나옴

SELECT * FROM EMPLOYEE WHERE EXISTS (SELECT 1 FROM EMPLOYEE WHERE EMP_ID = '2000'); -- RESULT SET이 안나오므로 EXISTS가 FALSE를 반환 -> 메인쿼리의 결과값 나오지 않음
...
SELECT 1 FROM EMPLOYEE WHERE DEPT_CODE = 'D5' AND EMP_NAME = '심봉선'; --나옴.
...
SELECT 1 FROM EMPLOYEE WHERE DEPT_CODE = 'D9' AND EMP_NAME = '심봉선';
-- 메인쿼리
SELECT DEPT_CODE, EMP_NAME 
FROM EMPLOYEE E 
