...
-- CREATE TRIGGER 트리거명
-- BEFORE (OR AFTER)
-- DELETE (OR UPDATE O INSERT) ON 테이블명
-- [FOR EACH ROW]
-- BEGIN
--      (실행문)
-- END;
...
CREATE OR REPLACE TRIGGER TRG_SALARY_NEW
AFTER
UPDATE ON EMPLOYEE
...
END;
/
-- Trigger TRG_SALARY_NEW이(가) 컴파일되었습니다.
...
(
    ...
    PCODE NUMBER, --CONSTRAINT FK_PRODUCT_IO REFERENCES PRODUCT(PCODE) FOREIGN KEY 컬럼레벨로 사용
    PDATE DATE,
    AMOUNT NUMBER,
    STATUS VARCHAR2(10) CHECK(STATUS IN ('입고', '출고')),
    CONSTRAINT FK_PRODUCT_IO FOREIGN KEY(PCODE) REFERENCES PRODUCT(PCODE) -- FOREIGN KEY 테이블레벨로 사용
);
SELECT * FROM PRODUCT_IO;

-- 3. 시퀀스는 SEQ_PRODUCT_PCODE, SEQ_PRODUCTIO_IOCODE라는 이름으로 기본값으로 설정되어있음.
CREATE SEQUENCE SEQ_PRODUCTIO_PCODE
START WITH 1
INCREMENT BY 1
NOCYCLE
NOCACHE;
...
-- 제품 출고 테이블 INSERT -> 입고이면 재고에 +1, 출고이면 재고에 -1
