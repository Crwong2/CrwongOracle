-- 8일차 PL/SQL
-- Oracle`s Procedual Language Extension to SQL 의 약자
...
BEGIN
    ...
    CASE INPUT
        ...
        WHEN 1 THEN DBMS_OUTPUT.PUT_LINE('노란색입니다');
        WHEN 1 THEN DBMS_OUTPUT.PUT_LINE('초록색입니다');
    END CASE;
END;
/
...
BEGIN
    ...
    LOOP
        N := N + 1;
        CONTINUE WHEN MOD(N, 2) = 0; -- 나머지가 0이면 짝수니까 출력하지 않고 반복(CONTINUE)으로
        DBMS_OUTPUT.PUT_LINE(N);
    END LOOP;
END;
/
...
-- @실습문제4
-- EMPLOYEE 테이블의 사번이 200 ~ 201인 직원들의 사원번호, 사원명, 이메일을 출력
DECLARE
    EINFO EMPLOYEE%ROWTYPE;
BEGIN
    FOR I IN 200..210
