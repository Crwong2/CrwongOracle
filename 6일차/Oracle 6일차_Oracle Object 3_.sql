...
/*
    1.1 개요
    - 실제 테이블에 근거한 논리적인 가상의 테이블
      (사용자에게 하나의 테이블 처럼 사용 가능하게 함)
    - SELECT 쿼리의 실행 결과를 화면에 저장한 논리적인 가상의 테이블
      (실질적인 데이터를 저장하고 있지 않지만 하나의 테이블처럼 사용 가능함)
...
-- 1.2.1 Stored View(저장O) - 이름을 붙여서 저장함.
    ...
    FROM EMPLOYEE;
-- 주의 : VIEW를 만들때에는 권한이 필요함, 권한명 : CREATE VIEW
-- System계정으로 권한 부여
...
/*
    1.3 특징
    - 테이블에 있는 데이터를 보여줄 뿐이며, 데이터 자체를 포함하고 있는 것은 아니다.
    - 저장장치 내에 물리적으로 존재하지 않고 가상 테이블로 만들어짐
    - 물리적인 실제 테이블과의 링크 개념임.
    - 컬럼에 산술연산을 하여도 VIEW 생성 가능
    - JOIN을 활용한 VIEW생성도 가능
    1.4 목적
...
-- 선동일의 DEPT_CODE를 DB로 바꾸는 DML을 작성하시오.
UPDATE V_EMPLOYEE
SET DEPT_CODE = 'D8'
WHERE EMP_ID = '200';

SELECT * FROM EMPLOYEE;
-- VIEW와 원본은 링크가 되어 있어서 VIEW 수정시 원본도 수정된다.
-- 확인했으면 ROLLBACK!
ROLLBACK;
...
-- 1.5.1 OR REPLACE
-- 수정하기
CREATE OR REPLACE VIEW V_EMPLOYEE  -- OR REPLACE : 수정
AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE;
-- 1.5.2 FORCE/NOFORCE
...
CREATE OR REPLACE FORCE VIEW V_EMPLOYEE  -- OR REPLACE : 수정
AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE WITH READ ONLY; -- 이런식으로 쓸 수 있다.
UPDATE V_EMPLOYEE
SET EMP_NAME = '선동열'
WHERE EMP_ID = '200';
...
LEFT OUTER JOIN DEPARTMENT ON DEPT_ID = DEPT_CODE WITH READ ONLY;
...
-- 마지막 번호가 몇번이었는지 몇씩 증가해서 들어가야 하는지를 기억하고 있어야함.
...
-- 2.2 Sequence 옵션
-- 2.2.1 MINVALUE : 발생시킬 최솟값 지정
-- 2.2.2 MAXVALUE : 발생시킬 최댓값 지정
...
-- SELECT 뒤 또는 INSERT INTO VALUES의 전달값으로 작성
...
CREATE SEQUENCE SEQ_KH_NONO
NOCYCLE
NOCACHE;

INSERT INTO KH_MEMBER
VALUES(SEQ_KH_NO.NEXTVAL, '홍길동', 20, SEQ_KH_NONO.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_KH_NO.NEXTVAL, '청길동', 30, SEQ_KH_NONO.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_KH_NO.NEXTVAL, '외길동', 40, SEQ_KH_NONO.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_KH_NO.NEXTVAL, '고길동', 50, SEQ_KH_NONO.NEXTVAL);
SELECT * FROM KH_MEMBER;

COMMIT;

-- 시퀀스 수정
-- ALTER 이용해서 옵션들을 수정하면 됌
ALTER SEQUENCE SEQ_KH_NO
