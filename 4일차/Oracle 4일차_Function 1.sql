...
SELECT LENGTH(EMAIL) FROM EMPLOYEE; --단일행함수
SELECT FLOOR(SYSDATE-HIRE_DATE) FROM EMPLOYEE; --단일행함수
...
SELECT SYSDATE "입대일자", ADD_MONTHS(SYSDATE, 18) "제대일자", ROUND((ADD_MONTHS(SYSDATE, 18)-SYSDATE)*3) "짬밥수"
FROM DUAL;
...
--	ex) 노옹철	no_hc
--	ex) 정중하	jung_jh
SELECT EMP_NAME "직원 이름", SUBSTR(EMAIL, 1, INSTR(EMAIL, '@')-1) "아이디"
, INSTR(EMAIL, '@') "@위치"
FROM EMPLOYEE;
...
--	ex) 전지연		1997년 3월
SELECT EMP_NAME "직원명"
, SUBSTR(EXTRACT(YEAR FROM HIRE_DATE), 1, 5)||'년'
|| SUBSTR(EXTRACT(MONTH FROM HIRE_DATE), 1, 5)||'월' "입사년월" -- 두개의 함수를 연결하기 위해서는 AND가 아니라 연결연산자 ||를 쓰도록 하자.
FROM EMPLOYEE;
...
--	* 주말도 포함 , 소수점 아래는 버림
SELECT EMP_NAME "직원명", HIRE_DATE "입사일", FLOOR(SYSDATE-HIRE_DATE) "오늘까지 근무일수"
FROM EMPLOYEE;
...
-- '19'||SUBSTR(EMP_NO, 1, 2)는 문자열이지만 자동형변환에 의해 연산이 적용됨.
FROM EMPLOYEE;
--10. 직원명, 직급코드, 연봉(원) 조회
--  단, 연봉은 ￦57,000,000 으로 표시되게 함
--     연봉은 보너스포인트가 적용된 1년치 급여임
SELECT EMP_NAME "직원명", JOB_CODE "직급코드", (SALARY*12)+NVL((SALARY*BONUS), 0) "연봉"
FROM EMPLOYEE;
...
--자동형변환 안되는 경우
--SELECT '1,000,000' - '500,000' FROM DUAL; --오류메세지 뜸. ORA-01722 : invalid number
SELECT TO_NUMBER('1,000,000', '9,999,999') - TO_NUMBER('500,000', '999,999') "계산결과"
FROM DUAL;

SELECT HIRE_DATE, EXTRACT(YEAR FROM HIRE_DATE)||'년'
||EXTRACT(MONTH FROM HIRE_DATE)||'월'
||EXTRACT(DAY FROM HIRE_DATE)||'일' "입사일자1"
, TO_CHAR(HIRE_DATE, 'YYYY"년"/MM"월"/DD"일"') "입사일자2"
FROM EMPLOYEE;
...
-- CASE 함수
-- 사용법 : CASE WHEN 변하는값 = 조건값 THEN 참일때 값 WHEN 변하는 값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 END
...
--SELECT DEPT_CODE, SUM(SALARY) FROM EMPLOYEE;  -- 오류발생 ORA-00937: not a single-group group function
...
SELECT DEPT_CODE "부서코드", SUM(SALARY) "급여 합계", FLOOR(AVG(SALARY)) "급여 평균", COUNT(SALARY) "인원수"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 1 ASC;
...
-- BONUS 컬럼의 값이 존재한다면 그 행을 1로 카운팅, 보너스를 지급받은 사원이 없는 부서도 있음을 확인할 수 있따.
...
SELECT JOB_CODE "직급코드", COUNT(*) "직급별 사원수", FLOOR(AVG(SALARY)) "평균 급여"
FROM EMPLOYEE
--WHERE JOB_CODE != 'J1'  -- 1
--WHERE JOB_CODE <> 'J1' -- 2
WHERE JOB_CODE NOT IN 'J1' -- 3   1, 2, 3이 다 같은 표현이다.
GROUP BY JOB_CODE
ORDER BY 1 ASC;
...
SELECT TO_CHAR(HIRE_DATE, 'YYYY') "입사년도", COUNT(*) FROM EMPLOYEE
WHERE JOB_CODE != 'J1'
...
, FLOOR(AVG(SALARY)) "평균 급여", SUM(SALARY) "급여 합계", COUNT(SALARY) "인원수" 
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여', '3', '남', '4', '여', '무');

-- 지금까지 한 것
-- 숫자 처리 함수, 문자 처리 함수, 날짜 처리 함수
-- 그룹함수 결과이지만 GROUP BY 를 쓰면 기준별로 결과가 나옴
SELECT DEPT_CODE, AVG(SALARY) FROM EMPLOYEE
GROUP BY DEPT_CODE;  -- 그룹함수는 GROUP BY 를 꼭 써야 함.
...
HAVING SUM(SALARY) >= 5000000  -- HAVING은 WHERE조건절의 WHERE와 동일한 역할을 하고, GROUP BY한 결과값에 조건을 걸때에 HAVING을 쓴다.
ORDER BY 1 ASC;
